...
export function useGetTodosByUserId(userId: string, options?: UseQueryOptions<ApiResponse<ExTodo[]>>) {
  const {
    data: todos,
    isLoading,
    isFetching,
    ...
    queryKey: todosKeys.allByUserId(userId),
    queryFn: () => TodosApi.getAllByUserId(userId),
    enabled: !!userId,
    ...options,
  });
